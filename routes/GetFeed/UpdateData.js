...
var UpdateData = {
  ...
  update_comments: async function (req, res) {
    ...
    for (i = 0; i < BrandIDList.length; i++) {
      await update_comment_BrandInfoTable(
        parseInt(BrandIDList[i]),
        input_comments
      );
    }
    GetTodayData.renderdata(req, res);
...
async function updateCrawlingFeedTable(FeedID) {
  try {
    ...
    console.log("while update CrawlingFeedTable : Check attribute");
    console.log(err);
  }
}
...
async function UpdateCrawlingFeed(BrandIDList) {
  ...
  for (var i = 0; i < BrandIDList.length; i++) {
    ...
    var tmp = dbData.Items;
    for (var j = 0; j < tmp.length; j++) {
      var FeedID = tmp[j].FeedID;
      await updateCrawlingFeedTable(FeedID);
    }
  }
}
