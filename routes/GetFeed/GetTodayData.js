    ...
    renderdata: async function (req, res) {
        ...
        for (var i = 0; i < brandLength; i++) {
            ...
            if (dbData[i].ReviewStatus == "Y") {
                reviewst_jsondata = '<td class="YesStatus">' + dbData[i].ReviewStatus + '</td>'
            } else {
                reviewst_jsondata = '<td class="NoStatus">' + dbData[i].ReviewStatus + '</td>'
            }
            var insta_url = 'http://www.instagram.com/' + dbData[i].instaID;
            jsondata += '<tr>'
                + '<td class="td_chk"><input type="checkbox" name="chkbox" value=' + dbData[i].brandID + '></td>'
                + '<td>' + dbData[i].brandName + '</td>'
                + '<td>' + dbData[i].UpdateFeedNum + '</td>'
                + '<td>' + dbData[i].TodayDownloadNum + '</td>'
                + '<td>' + dbData[i].FeedNum + '</td>'
                + '<td>' + dbData[i].NewFeedNum + '</td>'
                + '<td>' + dbData[i].DownloadNum + '</td>'
                + '<td>' + dbData[i].FollowerNum + '</td>'
                + '<td>' + '<a href="' + insta_url + '" class="fa fa-instagram"></a>' + '</td>'
                + reviewst_jsondata
                + '<td>' + dbData[i].Comment + '</td>' + '</tr>';
        }
